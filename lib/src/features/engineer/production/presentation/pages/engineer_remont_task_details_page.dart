...
class EngineerRemontTaskDetailsPage extends StatelessWidget {
  const EngineerRemontTaskDetailsPage({super.key});
  ...
  builder: (context, state) {
    return Scaffold(
      ...
      persistentFooterButtons: state.remontDetailsModel?.status!='4'?null: [
        Row(
          spacing: 16.0,
          children: [
            ...
            Expanded(child: AppElevatedButton(text: S.of(context).accept, onClick: (){
              context.pushNamed(AppRoutes.engineerClosingTask.name,pathParameters: {
                'id': state.remontDetailsModel?.id.toString()??'',
                'type': state.remontDetailsModel?.type??'',
                'result': state.remontDetailsModel?.resultRemont??''
              }).then((onValue){
                if(onValue!=null){
                  context.pop(1);
                }
