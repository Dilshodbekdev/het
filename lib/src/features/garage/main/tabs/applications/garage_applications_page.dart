...
class GarageApplicationsPage extends StatefulWidget {
  const GarageApplicationsPage({super.key});

  @override
  State<GarageApplicationsPage> createState() => _GarageApplicationsPageState();
}

class _GarageApplicationsPageState extends State<GarageApplicationsPage> with TickerProviderStateMixin {
                      ...
                      LazyLoadScrollView(
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, index) => index == state.applications.length
                              ...
                              : GarageApplicationItem(
                              model: state.applications[index],
                              onClick: () {
                                bloc.applicationDetails(state.applications[index].id??0);
                                context.pushNamed(AppRoutes.garageApplicationDetails.name).then((onValue){
                                  if(onValue!=null){
                                    bloc.applications(null);
                                  }
                                });
